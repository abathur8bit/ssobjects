 ...
 * to send data to the socket. You don't read from the socket, you wait until
 * TelnetServer#processSingleMessage(TelnetMessage) is called.
 * <p>
 * <b>Note</b> Writting currently is blocked, but not a huge priority to make
 * non blocked since most of the time it will go first try. Will address later.
 *
...
public class TelnetServerSocket {
    ...
    public TelnetServerSocket(SocketChannel sock) throws UnknownHostException {
        this();
        this.sock = sock;
        InetAddress addr = sock.socket().getInetAddress();

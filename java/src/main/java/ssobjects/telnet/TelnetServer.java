...
public abstract class TelnetServer implements Runnable
{
  //TODO if we get an IOException on send methods we should probably close this socket
    ...
    public TelnetServer(InetAddress host,int port,long idle) throws IOException {
    ...
    public void printAllExcept(String s,TelnetServerSocket except) throws IOException {
    ...
    public void stopServer() {
        //TODO LBP: Shutdown handlers before stopping server, or run() method can do it when it sees it is no longer running
        running.set(false);
    }
...
//        sock.key.cancel();
    ...
    /** Override this if you want to be informed when a socket has an error on it. 
     * Do not close or try writing to the socket, as 
     * @param sock
     */
    protected void socketHasError(TelnetServerSocket sock) {}
}

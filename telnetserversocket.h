...
class TelnetServerSocket : public SocketInstance
{
  ...
  public:
    ...
    virtual void    sendBuffer();                                           ///< Will attept to send any data in the outgoing buffer.
    ...
    bool            isDumpingData() const {return m_dumpData;}          ///< Returns if read operations will show all read data as a hex dump.
    void            setDumpData(const bool d) {m_dumpData = d;}      ///< Set if read operations should show data it reads.
    
  protected:
    void            rotateBuffer(char* pbuffer,unsigned32 nBuffSize,unsigned32 nBytesRotatingOut);
    virtual void    addPacketBuffer(const char* pszString);                 ///< Put data into the buffer
  ...
  protected:
    ...
    bool            m_dumpData;   ///< Show the number of bytes read, and a hex dump of the data read.
